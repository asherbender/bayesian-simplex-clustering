...
Python implementation of a Bayesian simplicial mixture of multi-variate t
distributions. The class implements methods for generating simulated data and
...
set- specific mixing proportions. An additional layer of latent variables
interface the documents' topics and words.
...
class BayesianSimplexClustering(object):
    ...
    def group(self, *group):
        ...
        # Check that the number of arguments is consistent with the size of the
        # model.
    ...
    def infer(self, obs, alpha=np.inf, nu=np.inf, initpost=True,
        ...
        if initpost:
            ...
            # Initialize the distributions over the model-specific parameters.
        ...
        for i in range(max(numiter)):
            ...
            for j in range(numsamp):
                ...
                for k in range(numcomp):
                    loglik[k, :], weight[j][k, :] = post.comp[k].loglik(obs[j],
                                                                        nu=nu)
            ...
            # Evaluate the lower bound on the marginal log-likelihood of the
            # data.
            ...
            for j in range(numsamp):
                ...
                # Update the posterior distributions over the sample-specific
                # parameters.
            ...
            for j in range(numgroup):
                ...
                # Update the posterior distributions over the model-specific
                # group parameters.
